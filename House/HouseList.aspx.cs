...
{
    public partial class HouseList : EntityListBase<H_houseinfor>
    {
        ...
        protected string z_bottom()
        {
            ...
            foreach (h_State hr in h_States)
            {
                if (hr.StateID == 1)
                {
                    if (CheckRolePermission("查看未审核"))
                        sb.Append("<li><a class=\"iconL\" href=\"House/HouseList.aspx?StateID=" + hr.StateID + "&NavTabId=" + NavTabId + "&selectPage=" + (gv.PageIndex + 1).ToString() + "&doAjax=true&HouseID={HouseID}\" rel=\"ids\" target=\"selectedTodo\" title=\"确定要操作吗？\"><span>" + hr.Name + "</span></a></li>");
                }
                else
                    sb.Append("<li><a class=\"iconL\" href=\"House/HouseList.aspx?StateID=" + hr.StateID + "&NavTabId=" + NavTabId + "&selectPage=" + (gv.PageIndex + 1).ToString() + "&doAjax=true&HouseID={HouseID}\" rel=\"ids\" target=\"selectedTodo\" title=\"确定要操作吗？\"><span>" + hr.Name + "</span></a></li>");
            }
            ...
            if (Current.RoleNames.Contains("信息"))
            {
                sb.Append("<li><a class=\"iconL\" href=\"House/HouseList.aspx?StateID=999&NavTabId=" + NavTabId + "&selectPage=" + (gv.PageIndex + 1) + "&doAjax=true&HouseID={HouseID}\" rel=\"ids\" target=\"selectedTodo\" title=\"确定要操作吗？\"><span>信息部审核</span></a></li>");
                sb.Append("<li><a class=\"iconL\" href=\"House/HouseList.aspx?istop=1&NavTabId=" + NavTabId + "&selectPage=" + (gv.PageIndex + 1) + "&doAjax=true&HouseID={HouseID}\" rel=\"ids\" target=\"selectedTodo\" title=\"确定要操作吗？\"><span>房源置顶</span></a></li>");
                sb.Append("<li><a class=\"iconL\" href=\"House/HouseList.aspx?istop=0&NavTabId=" + NavTabId + "&selectPage=" + (gv.PageIndex + 1) + "&doAjax=true&HouseID={HouseID}\" rel=\"ids\" target=\"selectedTodo\" title=\"确定要操作吗？\"><span>取消置顶</span></a></li>");
            }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                //        AreaGroupBtn += "<li><a class='add' href='House/HouseList.aspx?NavTabId=" + NavTabId + "&agroupId=0' target='navTab' rel='" + NavTabId + "' title='出售信息'><span>全部</span></a></li>";
                //        foreach (s_AreaGroup aGroup in aGroupList)
                //        {
                //            AreaGroupBtn += "<li><a class='add' href='House/HouseList.aspx?NavTabId=" + NavTabId + "&agroupId=" + aGroup.AreaGroupID + "' target='navTab' rel='" + NavTabId + "' title='" + aGroup.AreaGroupName + "'><span>" + aGroup.AreaGroupName + "</span></a></li>";
                //        }
                //    }
                //}
        ...
        protected override string GetWhereClauseFromSearchBar(string typeName)
        {
            ...
            //查看锁盘
            if (!CheckRolePermission("查看锁盘"))
            {
                sb.Append(" and IsLock=0");
            }
            ...
            //是否家庭唯一
            temp1 = GetMySearchControlValue("IsOnlyOne");
            if (temp1 == "on")
            {
                sb.AppendFormat(" and IsOnlyOne=1 ");
            }

            //是否有视频
            temp1 = GetMySearchControlValue("HasVideo");
            if (temp1 == "on")
            {
                sb.AppendFormat(" and isvideo=1 ");

                cHasVideo.Value = "on";
                myffrmHasVideo.Value = "on";
            }
            ...
            //委托单号
            temp1 = GetMySearchControlValue("orderNum");
            if (!temp1.IsNullOrWhiteSpace())
            {
                myffrmorderNum.Text = temp1;
